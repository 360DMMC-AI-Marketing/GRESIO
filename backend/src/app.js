...
app.use('/api/asana-import', require('./routes/asanaImport'));
app.use('/api/jira-import', require('./routes/jiraImport'));
app.use('/api/linear-import', require('./routes/linearImport'));
...
async function checkOverdueProjects() {
  try {
    ...
    for (const project of targetProjects) {
      ...
      const daysUntilDeadline = Math.ceil(msRemaining / 86400000);
      const isOverdue = msRemaining < 0;
      const tasks = await Task.find({ project: project._id, isActive: true, scope: 'project' });
      ...
      const isAtRisk = !isOverdue && daysUntilDeadline <= 7 && (overdueTasks > 0 || completionRate < 80);

      if (!isOverdue && !isAtRisk) continue;

      const subtype = isOverdue ? 'overdue_project' : 'at_risk_project';
      ...
      const daysStr = isOverdue
        ? `${Math.abs(daysUntilDeadline)} day(s) past its deadline`
        : `${daysUntilDeadline} day(s) remaining (${overdueTasks > 0 ? `${overdueTasks} overdue task(s)` : `${Math.round(completionRate)}% complete`})`;

      const notifData = {
        type: 'deadline_alert',
        title: isOverdue ? `Overdue Project: ${project.name}` : `At-Risk Project: ${project.name}`,
        message: `Project "${project.name}" is ${daysStr} (deadline: ${deadline.toLocaleDateString()})`,
        link: `/projects/${project._id}`,
        domain: project.domain,
        metadata: { projectId: String(project._id), subtype },
      };
      ...
      const admins = await User.find({ role: 'admin', domain: project.domain, _id: { $nin: recipients } }).select('_id');
      for (const a of admins) {
        if (!recipients.some(r => r.toString() === a._id.toString())) recipients.push(a._id);
      }
      if (recipients.length) {
...
app.post('/api/check-overdue-projects', auth, authorize('admin'), async (req, res) => {
  try {
    ...
    res.json({ message: 'Overdue/at-risk project check completed' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
