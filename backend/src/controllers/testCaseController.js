  ...
  try {
    ...
    if (stepResults) {
      for (const sr of stepResults) {
        ...
        if (step) {
          step.status = sr.status || step.status;
    ...
    tc.executionHistory.push({
      executionNumber,
      status: tc.status === 'passed' ? (tc.retestRequired ? 'retest_passed' : 'passed') : (tc.retestRequired ? 'retest_failed' : 'failed'),
    ...
    // On failure: auto-create bug with popup data
    if (tc.status === 'failed') {
    ...
    if (tc.status === 'passed' && tc.linkedBug) {
      // Close linked bug if test passes
      const Bug = require('../models/Bug');
      const bug = await Bug.findById(tc.linkedBug);
      if (bug && bug.status !== 'closed') {
        bug.status = 'fixed';
        bug.resolvedAt = new Date();
        bug.resolutionNotes = 'Auto-resolved — test passed on retest';
        await bug.save();
        // Notify
        const Notification = require('../models/Notification');
        if (bug.assignee) {
          await Notification.create({
            user: bug.assignee, domain: tc.project?.domain,
            type: 'status_change',
            title: `✅ Bug Resolved: ${bug.title}`,
            message: `Test passed — bug auto-resolved`,
