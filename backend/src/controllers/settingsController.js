...
exports.updateSettings = async (req, res, next) => {
  try {
    ...
    // Notify members when phase changes
    if (general?.phase && general.phase !== project.phase) {
      const memberIds = (project.members || []).filter(m => String(m._id || m) !== String(req.user._id));
      if (memberIds.length) {
        const notifDocs = memberIds.map(m => ({
          user: m._id || m, domain: req.user.domain,
          type: 'project_update',
          title: `📋 Phase Changed: ${project.name}`,
          message: `Project phase moved from "${(project.phase || '').replace(/_/g,' ')}" to "${general.phase.replace(/_/g,' ')}"`,
