          ...
          {list.length === 0 ? (
            ...
            return (
              <tr key={t._id} onClick={() => onSelect(t)} style={{cursor:'pointer'}}>
                <td onClick={e => e.stopPropagation()}><input type="checkbox" style={{accentColor:'#2347e8'}} /></td>
...
function SeparateTaskTable({ tasks: list, onSelect, onStatusChange, isManager, user }) {
  return (
    <div className="card">
      <table className="task-table">
        <thead><tr><th style={{width:28}}><input type="checkbox" style={{accentColor:'#2347e8'}} /></th><th>Task</th><th>Type</th><th>Status</th><th>Priority</th><th>Assignee</th><th>Assigned By</th><th>Deadline</th><th>Created</th></tr></thead>
        <tbody>
          {list.length === 0 ? (
            <tr><td colSpan={9} style={{textAlign:'center',padding:20,fontSize:11,color:'#9ca3af'}}>No separate tasks found</td></tr>
          ) : list.map((t) => {
            const isOverdue = t.deadline && new Date(t.deadline) < new Date() && t.status !== 'done';
