...
export default function CreateTemplate() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <h1 className="text-xl font-bold text-surface-900 mb-4">Create Template</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <label className="text-xs font-medium text-surface-700 mb-1 block">Name</label>
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2 outline-none focus:border-[#2347e8]" />
          </div>
          <div className="col-span-2">
            <label className="text-xs font-medium text-surface-700 mb-1 block">Description</label>
            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
              className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2 outline-none focus:border-[#2347e8]" rows={2} />
          </div>
          <div>
            <label className="text-xs font-medium text-surface-700 mb-1 block">Project Type</label>
            <select value={form.projectType} onChange={e => setForm({ ...form, projectType: e.target.value })}
              className="w-full text-sm border border-surface-200 rounded-lg px-3 py-2 outline-none bg-white">
          ...
          <div className="col-span-2">
            <label className="text-xs font-medium text-surface-700 mb-1 block">Tags (comma separated)</label>
        ...
        <div className="space-y-3">
          ...
          {form.phases.map((phase, phaseIdx) => (
            <div key={phaseIdx} className="border border-surface-200 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-3">
                <input value={phase.name} onChange={e => updatePhase(phaseIdx, 'name', e.target.value)}
                  placeholder={`Phase ${phaseIdx + 1} name`}
                  className="flex-1 text-sm border border-surface-200 rounded-lg px-3 py-1.5 outline-none focus:border-[#2347e8]" />
                ...
                {phase.tasks.map((task, taskIdx) => (
                  <div key={taskIdx} className="flex items-center gap-2">
                    ...
                    <input type="number" min="1" value={task.estimatedHours} onChange={e => updateTask(phaseIdx, taskIdx, 'estimatedHours', Number(e.target.value))}
                      className="w-16 text-xs border border-surface-200 rounded-lg px-2 py-1.5 outline-none focus:border-[#2347e8]" />
                    <span className="text-[9px] text-surface-400 w-10">hours</span>
